...
SINGLE_SEQ_MODE=$2  # note: `1` means single-sequence mode is enabled; all other integers (e.g., `0`) indicate otherwise

# validate if the second argument is a valid integer
re='^[0-1]+$'
if ! [[ $SINGLE_SEQ_MODE =~ $re ]]; then
  echo "Error: The second argument to RF2NA's inference script must be an integer ('1' or '0') indicating whether or not to run in single-sequence mode."
  exit 1
fi

# check if single-sequence mode was requested; if so, inform the user
if [ "$SINGLE_SEQ_MODE" -eq 1 ]; then
  echo "Single-sequence mode enabled. Performing structure predictions without MSAs."
fi

WDIR=`realpath -s $1`  # working folder
mkdir -p $WDIR/log

conda activate $PIPEDIR/RF2NA/
...
function proteinMSA {
    ...
    then
        ...
        echo " -> Running command: $PIPEDIR/input_prep/make_protein_msa.sh $seqfile $WDIR $tag $CPU $MEM $SINGLE_SEQ_MODE"
        $PIPEDIR/input_prep/make_protein_msa.sh $seqfile $WDIR $tag $CPU $MEM $SINGLE_SEQ_MODE > $WDIR/log/make_msa.$tag.stdout 2> $WDIR/log/make_msa.$tag.stderr
    fi
...
function RNAMSA {
    ...
    then
        ...
        echo " -> Running command: $PIPEDIR/input_prep/make_rna_msa.sh $seqfile $WDIR $tag $CPU $MEM $SINGLE_SEQ_MODE"
        $PIPEDIR/input_prep/make_rna_msa.sh $seqfile $WDIR $tag $CPU $MEM $SINGLE_SEQ_MODE > $WDIR/log/make_msa.$tag.stdout 2> $WDIR/log/make_msa.$tag.stderr
    fi
}

argstring=""

shift 2  # remove the first two input arguments to parse a variable number of FASTA input files
nP=0
nR=0
nD=0
for i in "$@"
